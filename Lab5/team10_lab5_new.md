...
Estimated values of N were calculated from the CMFR model while estimated values of Pe were calculated from the AD model. For the second experiment, which had one baffle placed in the reactor, N = 2.13 while Pe = 0.010. For the third experiment, which had two identical baffles placed in the reactor, N = 2.34 while Pe = 0.91.

While it is difficult to compare the values of N and Pe since only data collected two experiments were able to fit into the curve models, it appears that the N for the estimated number of reactors for the one baffle experiment is 2.19, which is expected, while the N for the two baffle experiment is 2.34. It is good that this number is larger than that for two baffles, but we expected it to be closer to 3. On the other hand, for the one baffle experiment, Pe = 0.010 while Pe = 0.91 for the two baffle experiment. This is expected as from Figure 3, it can be seen that the actual data in the second experiment follows the CMFR much more closely so there is less dispersion, whereas the issues with data collection resulted in a more calculated dispersion with the third experiment.
